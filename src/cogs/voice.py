...
class VoiceCog(commands.Cog):
    ...
    async def on_voice_state_update(
        ...
        # Handle join (lobby creation + join tracking + text channel sync)
        if (
            after.channel
            ...
            await self._sync_text_channel_join(member, after.channel)
        ...
        if (
            ...
            await self._sync_text_channel_leave(member, before.channel)
    ...
    async def _get_text_channel(
        ...
        """Get the linked text channel for a voice channel, if any."""
        async with async_session() as session:
            vs = await get_voice_session(
                session, str(voice_channel_id)
            )
            if not vs or not vs.text_channel_id:
                return None
        return guild.get_channel(int(vs.text_channel_id))

    async def _sync_text_channel_join(
        self, member: discord.Member, channel: discord.VoiceChannel
    ) -> None:
        """Grant text channel access when a member joins the VC."""
        tc = await self._get_text_channel(member.guild, channel.id)
        if tc:
    ...
    async def _sync_text_channel_leave(
        self, member: discord.Member, channel: discord.VoiceChannel
    ) -> None:
        """Revoke text channel access when a member leaves the VC."""
        tc = await self._get_text_channel(member.guild, channel.id)
        if tc:
            with contextlib.suppress(discord.HTTPException):
        ...
        async with async_session() as session:
            ...
            if len(channel.members) == 0:
                ...
                if voice_session.text_channel_id:
                    tc = channel.guild.get_channel(
                        int(voice_session.text_channel_id)
                    )
                    if tc:
                        with contextlib.suppress(discord.HTTPException):
                            await tc.delete(
                                reason="Ephemeral VC: 全員退出"
                            )
                with contextlib.suppress(discord.HTTPException):
